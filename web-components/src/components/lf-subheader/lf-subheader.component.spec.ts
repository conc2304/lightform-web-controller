...
describe("LfSubheader", () => {
    ...
    it("should set slot content", async () => {
        ...
        expect(page.root.querySelector(`.${slotStartClassName}`).innerHTML).toBe(slotStartContent);

        expect(page.root.querySelector(`.${slotEndClassName}`)).toBeTruthy();
        expect(page.root.querySelector(`.${slotEndClassName}`).getAttribute("src")).toBe(slotEndContent);
