  ...
  renderListItems() {
    return [
      this.wifiEntries.map((item, index) => {
        return (h("lf-wifi-list-item", { tabindex: "0", passwordProtected: item.passwordProtected, networkName: item.wifiName, signalStrength: item.signalStrength, index: index, focusElem: index === 0, style: { "--animation-order": index }, class: "wifi-list-item", onClick: () => this.onWifiEntryClicked(item) }));
      }),
      h("div", { onClick: () => this.getWifiList(), class: "wifi-list--refresh-list wifi-list-item", tabindex: "0", style: { "--animation-order": this.wifiEntries.length } }, h("div", null, "Refresh Wifi List")),
    ];
  }
